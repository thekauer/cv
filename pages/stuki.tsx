...
const Loop = styled.div`
    ...
    display:grid;
    place-content: center;
    &:hover,&:hover +div,&.selected,&.selected +div {
        background-color: var(--theme-light);
        cursor: pointer;
    }
    &.active, &.active +div{
...
const Code = ({content}:CodeProps) => {
    ...
    return (
        ...
        :
        <Input ref={inputRef} value={newContnet} onChange={(e)=>setNewContent(e.target.value)} onKeyDown={(e)=>{if(e.key=='Enter'){setEditing(false)}}} />
        }
        </>
    );
}
...
const NewItemMenu = ({editorState,setEditorState}:NewItemMenuProps) => {
    ...
    return (
        <>
            <StyledNewItemMenu tabIndex={0} id="NewItemMenu" onKeyDown={(e)=>keyDown(e)}>
                <Option className={selected==0?"selected":""} onClick={()=>click(0)}>
                    <span>Állítás</span>
                    <Statement></Statement>
                </Option>
                <Option className={selected==1?"selected":""} onClick={()=>click(1)}>
                    <span>Elágazás</span>
                    <IfElse></IfElse>
                    <IfElseContainer>
                        <If><Statement></Statement></If>
                        <Else><Statement></Statement></Else>
                    </IfElseContainer>
                </Option>
                <Option className={selected==2?"selected":""} onClick={()=>click(2)}>
                    <span>Ciklus</span>
                    <LoopContainer>
                        <Loop></Loop>
                        <LoopLeft/>
                        <LoopRight><Statement></Statement></LoopRight>
                    </LoopContainer>
                </Option>
            </StyledNewItemMenu>
        </>
...
const InfoMenu = () => {
    ...
    return (
        ...
        {examples.map(ex=>{
            return (
            <>
            <InfoLine><code>{ex}</code><Code content={ex}/></InfoLine>
            </>)
        })}
        <h4>Vezérlés</h4>
        <InfoLine>
        ...
        <h4>Beszúrás</h4>
        <InfoLine><KBD>CTRL</KBD>+<KBD>&uarr;</KBD></InfoLine>
        <InfoLine><KBD>CTRL</KBD>+<KBD>&darr;</KBD></InfoLine>
        <h4>Törlés</h4>
        <InfoLine><KBD>&nbsp;&nbsp;&larr;&nbsp;&nbsp;</KBD><KBD>Del</KBD></InfoLine>
        </>
    );
}
//#endregion
...
const Stuki = () => {
    ...
    const [editorState,setEditorState] = useState<EditorState>({
        functions:[{
            signature:{content:"main()",type:CodeType.Signature},
            body:[
                {content:"default",type:CodeType.Statement},
                {condition:"a==b",ifBranch:[{content:"",type:CodeType.Statement}],elseBranch:[{content:"",type:CodeType.Statement},{content:"",type:CodeType.Statement}],type:CodeType.IfElse} as IIfElse,
                {content:"",type:CodeType.Statement},
                {head:"i:=1..N",body:[{content:"",type:CodeType.Statement}],type:CodeType.Loop} as ILoop
            ]
        }],
        insertDirection:InsertDirection.None,
        editing:false,
    ...
    useEffect(()=>{
        const getNewStmt = () : ICode => {  
            switch(editorState.newType) {
                ...
                case CodeType.IfElse:
                    return {condition:"",ifBranch:[{content:"",type:CodeType.Statement}],elseBranch:[{content:"",type:CodeType.Statement}]} as IIfElse;
                case CodeType.Loop:
                    return {head:"",body:[{content:"",type:CodeType.Statement}]} as ILoop;
            }
        }
        if(editorState.newType) {
            ...
            editorState.functions[editorState.currentFunction].body.splice(y,0,newStmt);
    ...
    const editorKeyDown = (e:React.KeyboardEvent<HTMLDivElement>) => {
        ...
        const deleteElem = () => {
            ...
            if(isDeletingFunction) {
                ...
                //deleting code
                editorState.functions[editorState.currentFunction].body.splice(editorState.y-1,1);
                setEditorState({
                    ...editorState,
                    y:editorState.y-1
                })
        ...
        const selectElem = () => {
            let selected = document.getElementsByClassName('selected')[0]?.firstChild as HTMLDivElement;
            selected?.click();
        }
        const up = () => {
            if(e.ctrlKey) {
        ...
        const down = () => {
            if(e.ctrlKey) {
                ...
                if(editorState.y<editorState.functions[editorState.currentFunction].body.length) {
                    setEditorState({
                        ...editorState,
                        y:editorState.y+1
                    });
    ...
    const renderCode = (code : ICode, idx : number,fnIdx : number) => {
        ...
        switch(code.type){
            case CodeType.Statement: {
                return (<Statement className={isSelectedFn()&&idx+1==editorState.y?"selected":""}><Code content={code.content}/></Statement>)
            }
            case CodeType.IfElse: {
                return (<>
                    <IfElse className={isSelectedFn()&&idx+1==editorState.y?"selected":""}><Code content={(code as IIfElse).condition}/></IfElse>
                    <IfElseContainer>
                        <If>
            ...
            case CodeType.Loop: {
                return (
                    <>
                        <LoopContainer>
                            <Loop className={isSelectedFn()&&idx+1==editorState.y?"selected":""}><Code content={(code as ILoop).head}/></Loop>
                            <LoopLeft/>
                            <LoopRight>
    ...
    const renderFn = (fn : IFunction,fnIdx:number) => {
        ...
        return (
            ...
            {
                [<Signature className={isSelectedFn()&&editorState.y==0?"selected":""}><Code content={fn.signature.content}/></Signature>,
                <Line/>
    ...
    return (
        ...
        <MainContainer>
            <MainMenu>
                <MenuButton onClick={()=>activeMenu==0?setActiveMenu(-1) : setActiveMenu(0)} className={activeMenu==0?"active":""} ><Icon path="/static/play.svg"/></MenuButton>
                <MenuButton onClick={()=>activeMenu==1?setActiveMenu(-1) : setActiveMenu(1)} className={activeMenu==1?"active":""} ><Icon path="/static/export.svg"/></MenuButton>
                <MenuButton onClick={()=>activeMenu==2?setActiveMenu(-1) : setActiveMenu(2)} className={activeMenu==2?"active":""}><Icon path="/static/info.svg"/></MenuButton>
            </MainMenu>
            <EditorContainer>
                {editorState.insertDirection!=InsertDirection.None && <NewItemMenu editorState={editorState} setEditorState={setEditorState}/>}
                <SideMenu>
                    {activeMenu==0 && <SideMenuContainer><PlayMenu/></SideMenuContainer>}
                    {activeMenu==1 && <SideMenuContainer><ExportMenu/></SideMenuContainer> }
                    {activeMenu==2 && <SideMenuContainer><InfoMenu/></SideMenuContainer> }
                </SideMenu>
                <Editor ref={editorRef} tabIndex={0} onKeyDown={(e)=>editorKeyDown(e)}>
