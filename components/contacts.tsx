  ...
  const clickSubmit = (event: any) => {
    ...
    if (name.current && email.current && message.current) {
      ...
      db.collection("mail")
        .add(payload)
        .then((docRef) => {
          alert.success(t("sent_successfully"));
        })
        .catch((error) => {
          alert.error(t("error"));
        });
      ...
      <StyledContacts>
        <Content>
          <Side>
            ...
            <Ul>
              <Li>
                <Icon path="/static/email.svg" size="1.5em" />{" "}
                andraskauer@gmail.com
              </Li>
              <Li>
                <Icon path="/static/phone.svg" size="1.5em" />
                {t("phone")}: 06 20 409 5176
              </Li>
            </Ul>
            <Social>
              <a href="https://www.github.com/thekauer">
                <Icon path="/static/github.svg" />
              </a>
              <a href="https://www.linkedin.com/in/andr%C3%A1s-kauer-75967b208/">
                <Icon path="/static/linkedin.svg" />
              </a>
            </Social>
          </Side>
